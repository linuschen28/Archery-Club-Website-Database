...
<html lang="en-US">
	<head>
		...
		<title>Achery Club Main Page</title>
    ...
    <body class="wrapper">
        ...
        <section class="Member Register form">
            <form action="members_signup.php" method="POST">
	...
	function handleInsertMember()
	{
		global $db_conn;
		if (!empty($_POST['Birthday']) && !empty($_POST['email']) && !empty($_POST['name'])) {
			//Getting the values from user and insert data into the table
			$tuple = array(
				":bind1" => getNextMemberID(),
				":bind2" => $_POST['Birthday'],
				":bind3" => $_POST['email'],
				":bind4" => $_POST['name']
			);
			...
			executeSQLWithTuples("insert into Enlist2 values ('UBC Archery Club', 2023, :bind1)", $alltuples);
			oci_commit($db_conn);
			echo "SUCCESS";
		} else {
	...
	function getNextMemberID()
	{
		...
		$query = "SELECT member_id_seq.NEXTVAL FROM dual";
	...
	if (isset($_POST['name']) || isset($_POST['email']) || isset($_POST['Birthday']) || isset($_POST['ID'])) {
		handlePOSTRequest();
	}
